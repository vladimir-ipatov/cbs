...
partgen.sh -d DISKS [-v yes|no] -l [1|10|none]
  ...
  -l [LEVEL|none] - create RAID1/5/6/10 with lvm vg cbs and cbs/home
   LEVEL is the number, default: 1.
   If DISKS=1 then simple partition will be created instead of RAID.
   Use "none" if you wish to create LVM manually later - the "cbs-setup vg"
...
d-i partman-auto-lvm/new_vg_name string cbs
EOF
  fi
  cat <<EOF
...
EOF
  if [ -n "$partlvm" ]; then
  cat <<EOF
d-i preseed/late_command string lvremove -f /dev/cbs/removeit
EOF
  fi
  ;;
esac
